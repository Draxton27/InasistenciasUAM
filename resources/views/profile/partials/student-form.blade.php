...
@if ($estudiante)
    <form method="POST" action="{{ route('estudiante.profile.update') }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">CIF</label>
            <input type="text" name="cif" value="{{ old('cif', $estudiante->cif) }}"
                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white sm:text-sm">
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Email</label>
            <input type="email" name="email" value="{{ old('email', $estudiante->email) }}"
                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white sm:text-sm">
        </div>
        ...
        <div class="mb-4">
            ...
            @if ($estudiante->foto)
                ...
                <div id="foto-preview" class="relative inline-block mb-2">
                    <img src="{{ asset('storage/' . $estudiante->foto) }}" alt="Foto actual"
            ...
            <div id="upload-foto" style="{{ $estudiante->foto ? 'display: none;' : '' }}">
...
@else
    ...
    <p class="text-sm text-gray-500">No se encontró un registro de estudiante vinculado a este usuario.</p>
@endif
