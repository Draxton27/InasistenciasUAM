...
<form action="{{ $isEdit ? route('justificaciones.update', $justificacion->id) : route('justificaciones.store') }}"
      method="POST"
      enctype="multipart/form-data"
      class="p-6"
      novalidate
      data-mediator="true"> <!-- se necesita activar el form mediator -->
        ...
        <div class="justificacion-item bg-gray-50 dark:bg-gray-700 rounded-xl p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="justificaciones[0][profesor_id]" data-mediator="required"
                            class="profesor-select w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-[#009CA9] focus:ring-[#009CA9] shadow-sm" required>
                        <option value="">Selecciona un profesor</option>
                        @foreach($profesores as $profesor)
                            <option value="{{ $profesor->id }}" {{ ($isEdit && $justificacion->claseProfesor->profesor_id == $profesor->id) ? 'selected' : '' }}>
                                {{ $profesor->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="justificaciones[0][clase_profesor_id]" data-mediator="required"
                            class="clase-select w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-[#009CA9] focus:ring-[#009CA9] shadow-sm" required>
                        <option value="">Selecciona una clase</option>
                        @foreach($clases as $clase)
                            <option value="{{ $clase->id }}" {{ ($isEdit && $justificacion->clase_profesor_id == $clase->id) ? 'selected' : '' }}>
                                {{ $clase->clase->name }} (Grupo {{ $clase->grupo }})
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="mb-6">
                ...
                <input type="date" name="justificaciones[0][fecha]" data-mediator="required"
                       required max="{{ date('Y-m-d') }}"
                       value="{{ $isEdit ? $justificacion->fecha : '' }}"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Tipo de Constancia</label>
            <select name="tipo_constancia" data-mediator="required" required class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-[#009CA9] focus:ring-[#009CA9] shadow-sm">
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Archivo Adjunto</label>
            ...
            @endif
            <input id="archivo" name="archivo" type="file" data-mediator="required" accept=".pdf,.jpg,.jpeg,.png" class="block w-full text-sm text-gray-500 dark:text-gray-400" />
    ...
    <div class="flex flex-col sm:flex-row justify-center mb-6 gap-2 sm:gap-4 w-full mt-4">
        <a href="{{ route('justificaciones.index') }}" class="w-full sm:w-auto inline-flex items-center px-4 py-2 bg-gray-200 dark:bg-gray-700 border border-transparent rounded-md font-semibold text-xs text-gray-700 dark:text-gray-300 uppercase tracking-widest hover:bg-gray-300 dark:hover:bg-gray-600 focus:bg-gray-300 dark:focus:bg-gray-600 active:bg-gray-300 dark:active:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-300 dark:focus:ring-gray-600 focus:ring-offset-2 transition ease-in-out duration-150 justify-center">
            Cancelar
        </a>

        <button type="submit" class="w-full sm:w-auto inline-flex items-center px-4 py-2 bg-[#009CA9] border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest justify-center opacity-50 cursor-not-allowed">
...
<script src="{{ asset('js/form-mediator.js') }}"></script>
